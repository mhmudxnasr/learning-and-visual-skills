...
2. **Identify Context & Topic Mapping:**
   ...
   - **Dynamic Folder Sync:**
     - **Additions:** Create missing category/branch/leaf folders, `_index.md` files, and `z - 📎 Attachments` directories.
     - **Pruning:** If nodes were removed from `Map.canvas` and their directories in `07 - 🎓 Learning/` contain *only* `_index.md` (no user notes), remove the obsolete empty directory. Never touch folders with user notes.
...
8. **Canvas Node Linking (`Map.canvas`):**
...
9. **Recommendations Auto-Sync (`Recommendations  — Mahmood.md`):**
...
10. **Update Central Table of Contents (`00 - Table of Contents.md`):**
